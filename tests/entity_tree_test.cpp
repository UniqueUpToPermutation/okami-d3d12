...
TEST_F(EntityTreeTest, CannotReparentRootTest) {
	entity_t entity1 = world->CreateEntity();
	EXPECT_ANY_THROW(world->SetParent(kRoot, entity1));
...
TEST_F(EntityTreeTest, CannotRemoveRootTest) {
	EXPECT_ANY_THROW(world->RemoveEntity(kRoot));
...
TEST_F(EntityTreeTest, SetParentToInvalidEntityTest) {
	...
	EXPECT_ANY_THROW(world->SetParent(entity, invalidParent));
...
TEST_F(EntityTreeTest, SignalTest) {
	...
	auto parent1 = world->CreateEntity();
	auto parent2 = world->CreateEntity();
	auto child1 = world->CreateEntity(parent1);

	world->SetParent(child1, parent2); // Change parent to trigger parent change signal

	world->RemoveEntity(child1);
	world->RemoveEntity(parent1);
	world->RemoveEntity(parent2);
